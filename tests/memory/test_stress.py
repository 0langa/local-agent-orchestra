...
class TestStress:
    ...
    def test_vector_search_performance(self, tmp_path) -> None:
        ...
        engine = get_engine()
        bucket = backend._bucket("global", None)
        backend._vectors.setdefault(bucket, {})
        backend._metas.setdefault(bucket, {})
        # Bulk populate internal state directly, then save once
        for i in range(1000):
            text = f"document content {i} about various topics"
            backend._vectors[bucket][f"key_{i}"] = engine.encode(text)
            backend._metas[bucket][f"key_{i}"] = {"text": text}
